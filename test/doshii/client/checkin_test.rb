...
class CheckinTest < Minitest::Test
  ...
  def test_that_it_creates_checkin
    assert @checkin.body.respond_to?(:id)
  ...
  def test_that_it_allocates_a_table
    create_table
    assert @table.body.respond_to?(:id)
    assert @table.body.name   == '3'
  ...
  def test_that_it_updates_a_checkin
    VCR.use_cassette('checkin/create3') do
      @checkin3 = Doshii.checkin.create @location.body.id do |p|
    ...
    VCR.use_cassette('checkin/update') do
      checkin = Doshii.checkin.update @checkin3.body.id do |p|
        p[:status]    = new_status
        p[:updatedAt] = @checkin3.body.updatedAt
      end
